#!/usr/bin/env node
/**
 * Web Components Transformation Wrapper
 *
 * Wraps the BUMBA Web Components optimizer for use with Claude Code skills.
 *
 ...
 * 3. Calls the Web Components optimizer
 * 4. Writes output to .design/extracted-code/web-components/
 * 5. Updates metadata
 *
...
const WebComponentsComponentTransformer = require('../shared-modules/design-system/web-components-component-transformer');
...
async function transform() {
  ...
  console.log('=== Web Components Transformation ===\n');
  ...
  if (config.project.framework !== 'web-components') {
    console.error(`Error: Project configured for ${config.project.framework}, not web-components`);
    console.error('Update .design/config.json or use the correct transform skill.\n');
    process.exit(1);
  }

  console.log('Configuration loaded (framework: web-components)\n');
  ...
  console.log('Executing Web Components transformation...');

  const outputPath = path.join(projectPath, '.design', 'extracted-code', 'web-components');
  ...
  try {
    ...
    const frameworkOptions = config.transformers?.options?.['web-components'] || {};

    result = await WebComponentsOptimizer.transform(tokens, {
  ...
  if (frameworkOptions.generateStories) {
    ...
    try {
      await generateStoriesForProject(projectPath, 'web-components');
    } catch (error) {
  ...
  console.log('=== Web Components Transformation Complete ===\n');
  console.log(`Generated Files: ${generatedFiles.length}`);
  console.log('Output Location: .design/extracted-code/web-components/');
  console.log('');
  console.log('Next Steps:');
  console.log('  - Review tokens: .design/extracted-code/web-components/src/tokens/');
  console.log('  - Import CSS variables in your components');
  console.log('  - Use tokens in Shadow DOM styles');
...
async function transformComponent(componentId, options = {}) {
  ...
  console.log(`[transform-web-components] Starting transform for: ${componentId}`);

  // Step 0: Instantiate transformer with registry checking built-in
  const transformer = new WebComponentsComponentTransformer(projectPath, {
  ...
  if (transformResult.skipped) {
    console.log(`[transform-web-components] ✓ Component already transformed: ${transformResult.outputPath}`);
    return {
      success: true,
      skipped: true,
      componentId,
  ...
  console.log(`[transform-web-components] ✓ Component transformed: ${transformResult.outputPath}`);
  if (transformResult.storyPath) {
    console.log(`[transform-web-components] ✓ Story generated: ${transformResult.storyPath}`);
  }
  ...
  try {
    ...
    if (stateResult.success) {
      console.log(`[transform-web-components] Updated state: ${componentId} -> transformed`);
    } else {
      console.warn(`[transform-web-components] State update warning: ${stateResult.error}`);
    }
  } catch (stateError) {
    // State tracking errors should not break the transform
    console.warn(`[transform-web-components] State tracking error (non-fatal): ${stateError.message}`);
  }
...
async function transformAll(options = {}) {
  ...
  console.log(`[transform-web-components] Transforming ${componentIds.length} components...`);
  const results = await transformComponents(componentIds, options);

  const succeeded = results.filter(r => r.success).length;
  const failed = results.filter(r => !r.success).length;

  console.log(`[transform-web-components] Complete: ${succeeded} succeeded, ${failed} failed`);
