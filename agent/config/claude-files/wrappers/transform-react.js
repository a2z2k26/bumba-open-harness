#!/usr/bin/env node
/**
 * React Transformation Wrapper
 *
 * Wraps the BUMBA react optimizer for use with Claude Code skills.
 *
 ...
 * 3. Calls the react optimizer
 * 4. Writes output to .design/extracted-code/react/
 * 5. Updates metadata
 *
...
const ReactComponentTransformer = require('../shared-modules/design-system/react-component-transformer');
...
const { TransformStateUpdater, findComponentIdByName } = require('../../packages/@design-bridge/server/transform-state-updater');

async function transform() {
  const projectPath = process.cwd();

  console.log('=== React Transformation ===\n');
  ...
  if (config.project.framework !== 'react') {
    console.error(`❌ Error: Project configured for ${config.project.framework}, not react`);
    console.error('Update .design/config.json or use the correct transform skill.\n');
    process.exit(1);
  }

  console.log(`✓ Configuration loaded (framework: react)\n`);
  ...
  console.log('Executing React transformation...');

  const outputPath = path.join(projectPath, '.design', 'extracted-code', 'react');
  ...
  try {
    ...
    const frameworkOptions = config.transformers?.options?.['react'] || {};

    // Mode 1: Token-based batch transformation
    // This mode is for batch token generation (legacy support)
    console.log('⚠ Note: Token-based transformation is legacy mode');
    console.log('Use --component=ID for registry-based transformation\n');

    // For Mode 1, we just ensure tokens are available
    // The actual component transformation happens via transformComponent()
    result.files = [];
    console.log('Tokens loaded and available for component transformation');
  ...
  // Step 7: Generate Storybook stories (if enabled)
  const frameworkOptions = config.transformers?.options?.['react'] || {};
  if (frameworkOptions.generateStories) {
    console.log('📖 Generating Storybook stories...\n');
    try {
      await generateStoriesForProject(projectPath, 'react');
    } catch (error) {
      console.warn('⚠ Warning: Story generation failed:', error.message);
    }
  }

  // Step 8: Report results
  console.log('=== ✅ React Transformation Complete ===\n');
  console.log(`Generated Files: ${generatedFiles.length}`);
  console.log(`Output Location: .design/extracted-code/react/`);
  console.log('');
  console.log('Next Steps:');
  console.log('  - Review tokens: .design/extracted-code/react/tokens/');
  console.log('  - Import in components');
  console.log('  - Run Storybook: npm run storybook');
  console.log('');
}
...
async function transformComponent(componentId, options = {}) {
  ...
  console.log(`[transform-react] Starting transform for: ${componentId}`);
  ...
  const transformer = new ReactComponentTransformer(projectPath, {
    ...
    styledComponents: options.styledComponents !== false,
  ...
  if (transformResult.skipped) {
    console.log(`[transform-react] ✓ Component already transformed: ${transformResult.outputPath}`);
    return {
      success: true,
      skipped: true,
      componentId,
  ...
  console.log(`[transform-react] ✓ Component transformed: ${transformResult.outputPath}`);
  if (transformResult.storyPath) {
    console.log(`[transform-react] ✓ Story generated: ${transformResult.storyPath}`);
  }
  ...
  try {
    ...
    if (stateResult.success) {
      console.log(`[transform-react] Updated state: ${componentId} -> transformed`);
    } else {
      console.warn(`[transform-react] State update warning: ${stateResult.error}`);
    }
  } catch (stateError) {
    // State tracking errors should not break the transform
    console.warn(`[transform-react] State tracking error (non-fatal): ${stateError.message}`);
  }
...
async function transformAll(options = {}) {
  ...
  console.log(`[transform-react] Transforming ${componentIds.length} components...`);
  ...
  console.log(`[transform-react] Complete: ${succeeded} succeeded, ${failed} failed`);
