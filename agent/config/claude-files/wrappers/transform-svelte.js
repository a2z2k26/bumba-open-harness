#!/usr/bin/env node
/**
 * Svelte Transformation Wrapper
 *
 * Wraps the BUMBA svelte optimizer for use with Claude Code skills.
 *
 ...
 * 3. Calls the svelte optimizer
 * 4. Writes output to .design/extracted-code/svelte/
 * 5. Updates metadata
 *
...
const SvelteComponentTransformer = require('../shared-modules/design-system/svelte-component-transformer');
...
async function transform() {
  ...
  console.log('=== Svelte Transformation ===\n');
  ...
  if (config.project.framework !== 'svelte') {
    console.error(`❌ Error: Project configured for ${config.project.framework}, not svelte`);
    console.error('Update .design/config.json or use the correct transform skill.\n');
    process.exit(1);
  }

  console.log(`✓ Configuration loaded (framework: svelte)\n`);
  ...
  console.log('Executing Svelte transformation...');

  const outputPath = path.join(projectPath, '.design', 'extracted-code', 'svelte');
  ...
  try {
    ...
    const frameworkOptions = config.transformers?.options?.['svelte'] || {};

    result = await svelteOptimizer.transform(tokens, {
  ...
  console.log('=== ✅ Svelte Transformation Complete ===\n');
  console.log(`Generated Files: ${generatedFiles.length}`);
  console.log(`Output Location: .design/extracted-code/svelte/`);
  console.log('');
  console.log('Next Steps:');
  console.log('  - Review tokens: .design/extracted-code/svelte/tokens/');
...
async function transformComponent(componentId, options = {}) {
  ...
  console.log(`[transform-svelte] Starting transform for: ${componentId}`);

  // Step 0: Instantiate transformer with registry checking built-in
  const transformer = new SvelteComponentTransformer(projectPath, {
  ...
  if (transformResult.skipped) {
    console.log(`[transform-svelte] ✓ Component already transformed: ${transformResult.outputPath}`);
    return {
      success: true,
      skipped: true,
      componentId,
  ...
  console.log(`[transform-svelte] ✓ Component transformed: ${transformResult.outputPath}`);
  if (transformResult.storyPath) {
    console.log(`[transform-svelte] ✓ Story generated: ${transformResult.storyPath}`);
  }
  ...
  try {
    ...
    if (stateResult.success) {
      console.log(`[transform-svelte] Updated state: ${componentId} -> transformed`);
    } else {
      console.warn(`[transform-svelte] State update warning: ${stateResult.error}`);
    }
  } catch (stateError) {
    // State tracking errors should not break the transform
    console.warn(`[transform-svelte] State tracking error (non-fatal): ${stateError.message}`);
  }
...
async function transformAll(options = {}) {
  ...
  console.log(`[transform-svelte] Transforming ${componentIds.length} components...`);
  const results = await transformComponents(componentIds, options);

  const succeeded = results.filter(r => r.success).length;
  const failed = results.filter(r => !r.success).length;

  console.log(`[transform-svelte] Complete: ${succeeded} succeeded, ${failed} failed`);
