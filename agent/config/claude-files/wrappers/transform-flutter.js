#!/usr/bin/env node
/**
 * Flutter Transformation Wrapper
 *
 * Wraps the BUMBA flutter optimizer for use with Claude Code skills.
 *
 ...
 * 3. Calls the flutter optimizer
 * 4. Writes output to .design/extracted-code/flutter/
 * 5. Updates metadata
 *
...
const FlutterComponentTransformer = require('../shared-modules/design-system/flutter-component-transformer');
...
// Note: Flutter uses native previews, not Storybook - hook skips story generation
...
async function transform() {
  ...
  console.log('=== Flutter Transformation ===\n');
  ...
  if (config.project.framework !== 'flutter') {
    console.error(`❌ Error: Project configured for ${config.project.framework}, not flutter`);
    console.error('Update .design/config.json or use the correct transform skill.\n');
    process.exit(1);
  }

  console.log(`✓ Configuration loaded (framework: flutter)\n`);
  ...
  console.log('Executing Flutter transformation...');

  const outputPath = path.join(projectPath, '.design', 'extracted-code', 'flutter');
  ...
  try {
    ...
    const frameworkOptions = config.transformers?.options?.['flutter'] || {};

    result = await flutterOptimizer.transform(tokens, {
  ...
  console.log('=== ✅ Flutter Transformation Complete ===\n');
  console.log(`Generated Files: ${generatedFiles.length}`);
  console.log(`Output Location: .design/extracted-code/flutter/`);
  console.log('');
  console.log('Next Steps:');
  console.log('  - Review tokens: .design/extracted-code/flutter/lib/');
  console.log('  - Run app: flutter run');
  console.log('  - Build: flutter build');
  console.log('');
}
...
async function transformComponent(componentId, options = {}) {
  ...
  console.log(`[transform-flutter] Starting transform for: ${componentId}`);

  // Step 0: Instantiate transformer with registry checking built-in
  const transformer = new FlutterComponentTransformer(projectPath, {
  ...
  if (transformResult.skipped) {
    console.log(`[transform-flutter] ✓ Component already transformed: ${transformResult.outputPath}`);
    return {
      success: true,
      skipped: true,
      componentId,
  ...
  console.log(`[transform-flutter] ✓ Component transformed: ${transformResult.outputPath}`);
  if (transformResult.storyPath) {
    console.log(`[transform-flutter] ✓ Story generated: ${transformResult.storyPath}`);
  }
  ...
  try {
    ...
    if (stateResult.success) {
      console.log(`[transform-flutter] Updated state: ${componentId} -> transformed`);
    } else {
      console.warn(`[transform-flutter] State update warning: ${stateResult.error}`);
    }
  } catch (stateError) {
    // State tracking errors should not break the transform
    console.warn(`[transform-flutter] State tracking error (non-fatal): ${stateError.message}`);
  }
...
async function transformAll(options = {}) {
  ...
  console.log(`[transform-flutter] Transforming ${componentIds.length} components...`);
  const results = await transformComponents(componentIds, options);

  const succeeded = results.filter(r => r.success).length;
  const failed = results.filter(r => !r.success).length;

  console.log(`[transform-flutter] Complete: ${succeeded} succeeded, ${failed} failed`);
