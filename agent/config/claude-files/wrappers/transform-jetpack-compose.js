#!/usr/bin/env node
/**
 * Jetpack Compose Transformation Wrapper
 *
 * Wraps the BUMBA jetpack-compose optimizer for use with Claude Code skills.
 *
 ...
 * 3. Calls the jetpack-compose optimizer
 * 4. Writes output to .design/extracted-code/jetpack-compose/
 * 5. Updates metadata
 *
...
const JetpackComposeComponentTransformer = require('../shared-modules/design-system/jetpack-compose-component-transformer');
...
// Note: Jetpack Compose uses native @Preview, not Storybook - hook skips story generation
...
async function transform() {
  ...
  console.log('=== Jetpack Compose Transformation ===\n');
  ...
  if (config.project.framework !== 'jetpack-compose') {
    console.error(`❌ Error: Project configured for ${config.project.framework}, not jetpack-compose`);
    console.error('Update .design/config.json or use the correct transform skill.\n');
    process.exit(1);
  }

  console.log(`✓ Configuration loaded (framework: jetpack-compose)\n`);
  ...
  console.log('Executing Jetpack Compose transformation...');

  const outputPath = path.join(projectPath, '.design', 'extracted-code', 'jetpack-compose');
  ...
  try {
    ...
    const frameworkOptions = config.transformers?.options?.['jetpack-compose'] || {};

    result = await jetpackComposeOptimizer.transform(tokens, {
  ...
  console.log('=== ✅ Jetpack Compose Transformation Complete ===\n');
  console.log(`Generated Files: ${generatedFiles.length}`);
  console.log(`Output Location: .design/extracted-code/jetpack-compose/`);
  console.log('');
  console.log('Next Steps:');
  console.log('  - Review tokens: .design/extracted-code/jetpack-compose/');
  console.log('  - Add to Android Studio project');
...
async function transformComponent(componentId, options = {}) {
  ...
  console.log(`[transform-jetpack-compose] Starting transform for: ${componentId}`);

  // Step 0: Instantiate transformer with registry checking built-in
  const transformer = new JetpackComposeComponentTransformer(projectPath, {
  ...
  if (transformResult.skipped) {
    console.log(`[transform-jetpack-compose] ✓ Component already transformed: ${transformResult.outputPath}`);
    return {
      success: true,
      skipped: true,
      componentId,
  ...
  console.log(`[transform-jetpack-compose] ✓ Component transformed: ${transformResult.outputPath}`);
  if (transformResult.storyPath) {
    console.log(`[transform-jetpack-compose] ✓ Story generated: ${transformResult.storyPath}`);
  }
  ...
  try {
    ...
    if (stateResult.success) {
      console.log(`[transform-jetpack-compose] Updated state: ${componentId} -> transformed`);
    } else {
      console.warn(`[transform-jetpack-compose] State update warning: ${stateResult.error}`);
    }
  } catch (stateError) {
    // State tracking errors should not break the transform
    console.warn(`[transform-jetpack-compose] State tracking error (non-fatal): ${stateError.message}`);
  }
...
async function transformAll(options = {}) {
  ...
  console.log(`[transform-jetpack-compose] Transforming ${componentIds.length} components...`);
  const results = await transformComponents(componentIds, options);

  const succeeded = results.filter(r => r.success).length;
  const failed = results.filter(r => !r.success).length;

  console.log(`[transform-jetpack-compose] Complete: ${succeeded} succeeded, ${failed} failed`);
